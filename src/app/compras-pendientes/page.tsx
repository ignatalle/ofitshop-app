...
import { Loader2, PackageOpen, CheckCircle2, XCircle, Search, Store } from 'lucide-react';
...
export default function ComprasPendientesPage() {
  ...
  // States for marking as CONSEGUIDO
  ...
  const handleResolveSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newCostCents = resolveCost ? Math.round(parseFloat(resolveCost) * 100) : 0;
      if (newCostCents < 0) {
      ...
      // 1. Actualizar pending_purchases
      ...
      // 2. Modificar quirúrgicamente el costo en orders.items
      ...
      if (orderData && orderData.items) {
        const newItems = orderData.items.map((item: any) => {
          if (item.id === purchase.item_id) {
            return { ...item, wholesaleCost: newCostCents };
          }
          return item;
        });
    ...
    <div className="p-4 flex flex-col gap-6 max-w-3xl mx-auto w-full pb-24">
      <div className="flex items-center justify-between pt-2">
        <div>
          ...
          <p className="text-sm text-ofit-text-soft">
            Lista de prendas a conseguir para cumplir con pedidos
          </p>
        </div>
      </div>
              ...
              <div key={supplierName} className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-3">
                  {groupPurchases.map(p => (
                    <div key={p.id} className={`bg-white rounded-2xl p-4 border transition-all ${p.status === 'PENDIENTE' ? 'border-gray-200 shadow-sm hover:border-gray-300' : 'border-gray-100 opacity-60'}`}>
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            ...
                            {p.customer?.name && (
                              <Link href={`/clientes?expand=${p.customer_id}`} className="text-xs font-bold text-ofit-pink bg-ofit-pink/10 hover:bg-ofit-pink/20 px-2 py-1 rounded-md transition-colors">
      ...
      {resolvingId && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl p-6 pb-10 sm:pb-6 animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:zoom-in-95 shadow-xl">
            <h2 className="text-xl font-bold text-ofit-text mb-2">Prenda Conseguida</h2>
            <p className="text-sm text-gray-500 mb-6">Confirmá el costo real que pagaste al mayorista. Esto actualizará las finanzas del pedido original.</p>
            ...
            <form onSubmit={handleResolveSubmit} className="flex flex-col gap-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required autoFocus
                    value={resolveCost} onChange={(e) => setResolveCost(e.target.value)}
              ...
              <div className="flex gap-3 mt-2">
                <button
                  ...
                  className="flex-1 h-12 font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isSubmitting || !resolveCost}
